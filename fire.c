...
object_list_t **top;
	...
	while (now)
		...
		old = now;
		now = now->next;
		if (old->object->done_count >= old->object->part_count ||
		    old->object->life < 0)
...
static object_t *BuildObject()
	...
	object->particles = (particle_t *)malloc(object->part_count *
	    sizeof(particle_t));
